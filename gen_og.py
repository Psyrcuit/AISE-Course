"""Generate OG images + apple-touch-icon for the AISE 26 course.

Outputs:
  - og.png        (1200x630, landscape - primary OG for FB/X/LinkedIn)
  - og.jpg        (1200x630, JPEG - LinkedIn-safe fallback)
  - og-square.png (1200x1200, square - for WhatsApp/iMessage previews)
  - apple-touch-icon.png (180x180, iOS bookmark)

Same toolchain as mirror/gen_og.py so the look is consistent across the
portfolio. Logo is rasterized via headless Chrome (Windows ARM64 has no
cairosvg). Run from this directory: `python gen_og.py`.
"""
import os
import subprocess
import tempfile
...
def build_canvas(W, H, *, brand_size, brand_y, head_size, head_lines,
    ...
    # Concept chips - a sampler of what's in the course.
    chips_font = font("segoeui.ttf", chip_size)
    if chips is None:
        chips = ["RAG", "agents", "MCP", "evals",
                 "voice agents", "context engineering"]
    widths = []
    for c in chips:
    ...
    draw.text((W // 2, H - url_y_from_bottom), "aise.psyrcuit.com",
...
landscape = build_canvas(
    ...
    head_size=68, head_lines=["514 concepts. 16 modules.",
                              "One map."],
    head_y=150, line_h=84,
    chip_size=26, chip_h=54, chip_pad_x=24, chip_gap=14,
    chips_y=400, url_size=30, url_y_from_bottom=46,
    max_row_w_pad=120,
)
landscape.save("og.png", "PNG", optimize=True)
print("og.png written - 1200x630 (landscape)")
# LinkedIn safe JPEG: baseline, no progressive, 4:2:0 chroma, no profile.
...
square = build_canvas(
    ...
    head_size=96, head_lines=["514 concepts.", "16 modules.",
                              "One map."],
...
# PWA icons: 192 + 512, full-bleed (logo nearly edge-to-edge, dark bg).
# Android home-screen icons get masked into circles/squircles; "maskable"
# safe zone is ~80% of the canvas, so we sit the logo at ~85% to feel
# substantial without getting cropped.
for size in (192, 512):
    canvas = Image.new("RGB", (size, size), BG)
    inset = int(size * 0.85)
    logo_img = render_logo(inset)
    rgba = canvas.convert("RGBA")
    off = (size - inset) // 2
    rgba.alpha_composite(logo_img, (off, off))
    rgba.convert("RGB").save(f"icon-{size}.png", "PNG", optimize=True)
    print(f"icon-{size}.png written - {size}x{size}")
